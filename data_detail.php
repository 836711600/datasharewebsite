...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")&&(!empty($_FILES['upfile']['name']))) {
  $insertSQL = sprintf("INSERT INTO upfile (name,savename, `size`, type, dataid) VALUES (%s,%s, %s, %s, %s)",
                       GetSQLValueString($_FILES['upfile']['name'], "text"),
                       GetSQLValueString(basename($_FILES['upfile']['tmp_name']), "text"),
  ...
  $path1='files/'.$_POST['dataid'].'/';
   if (is_uploaded_file($_FILES['upfile']['tmp_name'])) {
		$localfile = $path1 . iconv("UTF-8", "GBK", basename($_FILES['upfile']['tmp_name']));
    //$localfile = $path1 .$_FILES['upfile']['name'];
  ...
  $insertGoTo = "data_detail.php?id=".$row_Recordset1['dataid'];
  if (isset($_SERVER['QUERY_STRING'])) {
...
<table width="100%" border="1">
  ...
  <?php if($totalRows_Recordset2>0){do { ?>
    <tr>
      ...
      <td align="center"><a href="deletefile.php?id=<?php echo $row_Recordset2['fileid']; ?>&dataid=<?php echo $row_Recordset1['dataid']; ?>">删除</a></td>
    </tr>
    <?php } while ($row_Recordset2 = mysql_fetch_assoc($Recordset2));}; ?>
</table>
<p><a href="mydata.php">返回</a></p>
</body>
</html>
<?php
