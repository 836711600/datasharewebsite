...
for($i=0;$i<$row_num;$i++)
{
	$html.="<a href=\"http://catalog.data.gov/dataset/".$result[$i]->name."\" target=\"_blank\"><h3>".($i+1).".".$chinese[$i*3]->dst."</h3></a><p><b>来源：</b>".$chinese[$i*3+1]->dst."<br><b>描述：</b>".$chinese[$i*3+2]->dst."<br><b>数据格式：</b>";
...
$html.="<h2>";
$html.="<a href=\"data_gov.php?keyword=".$_GET["keyword"]."&page=1\">首页</a>&nbsp;";
if($current_page>1)
{
	$html.="<a href=\"data_gov.php?keyword=".$_GET["keyword"]."&page=".($current_page-1)."\"><前一页</a>&nbsp;";
}
for($j=0;$j<10&&(($i+$j)<=$total_page);$j++)
{
	...
	{ 
		$html.="<a href=\"data_gov.php?keyword=".$_GET["keyword"]."&page=".($i+$j)."\">".($i+$j)."</a>&nbsp;";	
	}
}
if($current_page<$total_page)
{
	$html.="<a href=\"data_gov.php?keyword=".$_GET["keyword"]."&page=".($current_page+1)."\">后一页></a>&nbsp;";
}
$html.="<a href=\"data_gov.php?keyword=".$_GET["keyword"]."&page=".$total_page."\">末页</a>&nbsp;";
$html.="共".$total_page."页";
$html.="</h2>";
echo $html;
?>
